...
public class Controller implements ActionListener, WindowListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		//Actions performed in start window
		...
		if (e.getSource() instanceof ButtonDuelMode){
			...
			try {
				if(!(gui.getStartWindow().getTxtFldPlayer1().getText().equals("") || gui.getStartWindow().getTxtFldPlayer1().getText().equals(" ")))
					p1 = new Player(gui.getStartWindow().getTxtFldPlayer1().getText());
				else
					p1 = new Player("Yugi");
				
				if(!(gui.getStartWindow().getTxtFldPlayer2().getText().equals("") || gui.getStartWindow().getTxtFldPlayer2().getText().equals(" ")))
		...
		//End actions performed in start window
		
		//Actions performed in game window
		...
		if (e.getSource() instanceof ButtonNextPhase){
			...
			if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){
				...
				if (board.getActivePlayer().getField().getPhase() != Phase.MAIN2){
					...
					gui.getGameWindow().getLblPlayer1Phase().setText(board.getActivePlayer().getField().getPhase() + "");
					gui.getGameWindow().getLblPlayer2Phase().setText(board.getOpponentPlayer().getField().getPhase() + "");
				}
		...
		//Summoning & setting monsters; activating & setting spells
		...
		if (firstClick == null){
			...
			} //End firstClick is ButtonHand
			
			else { 
				
				if (e.getSource() instanceof ButtonSpell){ //Activating spell in field
					...
					if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){
						...
						if (!(spell instanceof MagePower || spell instanceof ChangeOfHeart)){ 
							...
							try {	
								...
								JOptionPane.showMessageDialog(null, "You can't activate this spell in the battle phase");
							}
							...
							if(spell instanceof MagePower){
								...
								if(board.getOpponentPlayer().getField().getMonstersArea().size() > 0 || board.getActivePlayer().getField().getMonstersArea().size() > 0)
					
									JOptionPane.showMessageDialog(null, "Choose the monster for the spell to be acted upon");
					...
					if (e.getSource() instanceof ButtonMonster){ //Switching monster mode & declaring attack in field
						...
						if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){
							...
							if(board.getActivePlayer().getField().getPhase() != Phase.BATTLE){
								...
								int dialogResult = JOptionPane.showConfirmDialog(null, "Toggle mode?", "Toggle Mode", dialogButton);
								...
								if(dialogResult == JOptionPane.YES_OPTION){
									...
									if (!opSuccessful)
										...
										JOptionPane.showMessageDialog(null, "You can't change the mode of the monster again this turn");
								...
								if (board.getOpponentPlayer().getField().getMonstersArea().size() == 0){
									...
									try{
										...
										JOptionPane.showMessageDialog(null, "You cannot attack with a monster in defense mode");
									}
									catch(ArrayIndexOutOfBoundsException c){
										nullifyAttributes();
									}
			...
			if(secondClick == null){ //Actions requiring 2, 3 or 4
				
				if(e.getSource() instanceof ButtonHand){ //Hand monster/spell selection
					...
					if(secondClick.getActionCommand().equals("Summon Monster")){ //Summon monster from hand
						...
						if(secondClick.getActionCommand().equals("Set Monster")){ //Set monster from hand
							...
							if(secondClick.getActionCommand().equals("Set Spell")){ //Set spell from hand
								...
								try{
									...
									JOptionPane.showMessageDialog(null, "You cannot add this spell in the battle phase");
								}
								...
								if(secondClick.getActionCommand().equals("Activate Spell")){ //Activate spell from hand
									...
									if(!(spell instanceof ChangeOfHeart || spell instanceof MagePower)){
										...
										try{
											...
											JOptionPane.showMessageDialog(null, "You cannot activate this spell in the battle phase");
										}
										finally{
											nullifyAttributes();
										}
										...
										if (spell instanceof MagePower){
											...
											if (board.getActivePlayer().getField().getMonstersArea().size() > 0 || board.getOpponentPlayer().getField().getMonstersArea().size()>0)
												JOptionPane.showMessageDialog(null, "Choose the monster for the spell to be acted upon");
									...
									if (firstClick instanceof ButtonSpell){  
										...
										if(e.getSource() instanceof ButtonMonster){ //ChangeOfHeart & MagePower (activation from field) monster selection
											...
											if(spell instanceof ChangeOfHeart){
												...
												if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText())
													index = gui.getGameWindow().getArrayPlayer2Monsters().indexOf(secondClick);
													
												else
													index = gui.getGameWindow().getArrayPlayer1Monsters().indexOf(secondClick);
												...
												if(index == -1)
													...
													monsterSpelled = board.getOpponentPlayer().getField().getMonstersArea().get(index);
											...
											if(spell instanceof MagePower){ 
												...
												if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){
													...
													if(index != -1)
														monsterSpelled = board.getActivePlayer().getField().getMonstersArea().get(index);
										...
										else{ //if a spell is passed to ChangeOfHeart/MagePower instead of a monster (activation on field)
										...
										if (firstClick instanceof ButtonMonster){ 
											...
											if(e.getSource() instanceof ButtonMonster){ //Attacking monster on field
												...
												if(index != -1){
													...
													MonsterCard monsterAtt = board.getOpponentPlayer().getField().getMonstersArea().get(index);
												...
												if (e.getSource() instanceof ButtonSpell){
													...
													JOptionPane.showMessageDialog(null, "You cannot attack a spellcard");
													nullifyAttributes();
			...
			else{ //actions requiring 3 or 4 clicks
				...
				if(secondClick.getActionCommand().equals("Summon Monster")){
					...
					if(monster.getLevel() == 5 || monster.getLevel() == 6){
						...
						if(monster.getLevel() == 7 || monster.getLevel() == 8){
								...
								if(thirdClick == null){ //receiving first sacrifice
								...
								else{ //receiving second sacrifice
					...
					if(secondClick.getActionCommand().equals("Set Monster")){
					...
					else{ //Activation of MagePower & ChangeOfHeart from hand
						...
						if(secondClick.getActionCommand().equals("Activate Spell")){
								...
								if(thirdClick == null){
									...
									else{ //if a spell is passed to ChangeOfHeart/MagePower instead of a monster (activation from hand)
		...
		//Updating life points; deck count; graveyard, monsters area & spell area icons each click
		
		if (board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){ //updating player 1
			
			//Life points & deck count
			...
			gui.getGameWindow().getLblPlayer1DeckCount().setText("" + board.getActivePlayer().getField().getDeck().getDeck().size());
			gui.getGameWindow().getLblPlayer2DeckCount().setText("" + board.getOpponentPlayer().getField().getDeck().getDeck().size());
			
			gui.getGameWindow().getProgressBarPlayer2().setValue(Integer.parseInt(gui.getGameWindow().getLblPlayer2LifePoints().getText()));
			gui.getGameWindow().getProgressBarPlayer1().setValue(Integer.parseInt(gui.getGameWindow().getLblPlayer1LifePoints().getText()));
			
			//Graveyard
			
			if(board.getActivePlayer().getField().getGraveyard().size() == 0)
				gui.getGameWindow().getBtnPlayer1Graveyard().setIcon(new ImageIcon("CardBack.png"));
			
			else{
				
				Card card = board.getActivePlayer().getField().getGraveyard().get(board.getActivePlayer().getField().getGraveyard().size() - 1);
				gui.getGameWindow().getBtnPlayer1Graveyard().setIcon(new ImageIcon(card.getName() + ".png"));
				...
				gui.getGameWindow().getBtnPlayer1Graveyard().setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
				
				}
			
			if(board.getOpponentPlayer().getField().getGraveyard().size() == 0)
				gui.getGameWindow().getBtnPlayer2Graveyard().setIcon(new ImageIcon("CardBack.png"));
		
			else{
				
				Card card = board.getOpponentPlayer().getField().getGraveyard().get(board.getOpponentPlayer().getField().getGraveyard().size()-1);
				gui.getGameWindow().getBtnPlayer2Graveyard().setIcon(new ImageIcon(card.getName() + ".png"));
				...
				gui.getGameWindow().getBtnPlayer2Graveyard().setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
			...
			for(int i = 0; i < board.getActivePlayer().getField().getMonstersArea().size(); i++){
				...
				if (card.getMode() == Mode.DEFENSE)
					gui.getGameWindow().getArrayPlayer1Monsters().get(i).setIcon(new ImageIcon("CardBack.png"));
				
				else
					gui.getGameWindow().getArrayPlayer1Monsters().get(i).setIcon(new ImageIcon(cardName + ".png"));
				
				gui.getGameWindow().getArrayPlayer1Monsters().get(i).setEnabled(true);
				text = card.toString2();
				gui.getGameWindow().getArrayPlayer1Monsters().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
			...
			for(int i = 0; i < board.getOpponentPlayer().getField().getMonstersArea().size(); i++){
				...
				if (card.getMode() == Mode.DEFENSE){
					...
					gui.getGameWindow().getArrayPlayer2Monsters().get(i).setIcon(new ImageIcon("CardBack.png"));
					...
					gui.getGameWindow().getArrayPlayer2Monsters().get(i).setIcon(new ImageIcon(cardName + ".png"));
					gui.getGameWindow().getArrayPlayer2Monsters().get(i).setEnabled(true);
					text = card.toString2();
					gui.getGameWindow().getArrayPlayer2Monsters().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
			...
			for(int i = 0; i < board.getActivePlayer().getField().getSpellArea().size(); i++){
				...
				gui.getGameWindow().getArrayPlayer1Spells().get(i).setIcon(new ImageIcon("CardBack.png"));
				...
				text = card.toString();
				gui.getGameWindow().getArrayPlayer1Spells().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
			...
			for(int i = 0; i < board.getOpponentPlayer().getField().getSpellArea().size(); i++){
				...
				gui.getGameWindow().getArrayPlayer2Spells().get(i).setIcon(new ImageIcon("CardBack.png"));
				gui.getGameWindow().getArrayPlayer2Spells().get(i).setEnabled(true);
				
				String text = "";
				
				gui.getGameWindow().getArrayPlayer2Spells().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
		
			}
			
		}
		
		else{ //updating player 2
			...
			gui.getGameWindow().getLblPlayer2DeckCount().setText("" + board.getActivePlayer().getField().getDeck().getDeck().size());
			gui.getGameWindow().getLblPlayer1DeckCount().setText("" + board.getOpponentPlayer().getField().getDeck().getDeck().size());
		...
		if (e.getSource() instanceof ButtonDeck){
			 ...
			 int dialogButton = JOptionPane.YES_NO_OPTION;
			 int dialogResult = JOptionPane.showConfirmDialog(null, "Do you wish to surrender?", "Surrender", dialogButton);
		...
		//End actions performed in game window
		
		
		//Actions performed in end window
		...
		//End actions performed in end window
	...
	public void updateHand(){
		...
		if(board.getActivePlayer().getName() == gui.getGameWindow().getLblPlayer1Name().getText()){
			...
			for(int i = 0; i < board.getActivePlayer().getField().getHand().size(); i++){
				...
				gui.getHandPlayer1().getArrayCards().get(i).setIcon(new ImageIcon(cardName + ".png"));
				...
				gui.getHandPlayer1().getArrayCards().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
			...
			for(int i = 0; i < board.getActivePlayer().getField().getHand().size(); i++){
				...
				String cardName = board.getActivePlayer().getField().getHand().get(i).getName();
				gui.getHandPlayer2().getArrayCards().get(i).setIcon(new ImageIcon(cardName + ".png"));
				...
				gui.getHandPlayer2().getArrayCards().get(i).setToolTipText("<html><p width=\"250\">" + text + "</p></html>");
