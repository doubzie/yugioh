...
public class MonsterCard extends Card {
	...
	public String toString() {
		return super.toString() + ", ATK: " + attackPoints + ", DEF: " + defensePoints + ", LVL: " + level + ", "
				+ getDescription() + "\n";
	}

	public String toString2() {
		return super.toString() + ", ATK: " + attackPoints + ", DEF: " + defensePoints + ", LVL: " + level + ", " + mode
				+ ", " + getDescription() + "\n";
	}
	...
	public void action(MonsterCard monster) {
		...
		if (monster.getMode() == Mode.ATTACK) {
			...
			if (attackingPoints > attackedPoints) {
				...
				getBoard().getOpponentPlayer()
						.setLifePoints(getBoard().getOpponentPlayer().getLifePoints() - (deductedPoints));
				...
				if (attackingPoints == attackedPoints) {
					...
					getBoard().getActivePlayer()
							.setLifePoints(getBoard().getActivePlayer().getLifePoints() - (deductedPoints));
					getBoard().getActivePlayer().getField().removeMonsterToGraveyard(this);
				}
			}
	...
	public void action() {
		...
		if (Card.getBoard().getOpponentPlayer().getField().getMonstersArea().size() == 0) {
			...
			getBoard().getOpponentPlayer()
					.setLifePoints(getBoard().getOpponentPlayer().getLifePoints() - this.getAttackPoints());
			Card.getBoard().assignWinner();
