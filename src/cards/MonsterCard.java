...
public class MonsterCard extends Card {
    ...
    if (monster.getMode() == Mode.ATTACK) {
      ...
      if (attackingPoints > attackedPoints) {
        getBoard()
          .getOpponentPlayer()
          .setLifePoints(
            getBoard().getOpponentPlayer().getLifePoints() - (deductedPoints)
          );
        getBoard()
          .getOpponentPlayer()
          .getField()
        ...
        if (attackingPoints == attackedPoints) {
          ...
          getBoard()
            .getActivePlayer()
            .setLifePoints(
              getBoard().getActivePlayer().getLifePoints() - (deductedPoints)
            );
          getBoard()
            .getActivePlayer()
            .getField()
      ...
      if (attackingPoints > attackedPoints) getBoard()
        .getOpponentPlayer()
        .getField()
        .removeMonsterToGraveyard(monster); else {
        if (attackingPoints < attackedPoints) getBoard()
          .getActivePlayer()
          .setLifePoints(
            getBoard().getActivePlayer().getLifePoints() - (deductedPoints)
  ...
  public void action() {
    if (
      Card.getBoard().getOpponentPlayer().getField().getMonstersArea().size() ==
      0
    ) {
      getBoard()
        .getOpponentPlayer()
